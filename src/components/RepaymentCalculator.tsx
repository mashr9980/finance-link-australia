...
import { IframeModal } from '../hooks/iframeModel';
...
export function RepaymentCalculator() {
  ...
  const { isOpen, openModal, closeModal } = useIframeModal();
  ...
  return (
    <>
            ...
            <div className={`calculator-form transition-all duration-700 delay-200 ${isAnimated ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              <div className="dark-card rounded-2xl shadow-2xl backdrop-blur-sm p-8 border border-teal-500/20">
                ...
                <div className="result-section border-t border-teal-500/30 pt-6">
                  ...
                  <button 
                    onClick={openModal}
                    className="gradient-btn w-full py-4 text-white rounded-md transition-all font-medium hover:shadow-lg hover:shadow-teal-600/20"
                  >
                    Apply Now
                  </button>
                </div>
      ...
      <IframeModal
        isOpen={isOpen}
        onClose={closeModal}
        src="https://financelinkaustralia.ambitioncloud.com.au/s/7ft0rSTWF4"
